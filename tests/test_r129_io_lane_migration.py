...
from services.chatops.transport_contract import DeliveryMessage, DeliveryTarget, TransportType
...
class TestR129IOLaneMigration(unittest.IsolatedAsyncioTestCase):
    async def test_callback_delivery_uses_io_lane_for_history_and_http(self):
        ...
        with (
            ...
            patch.object(callback_delivery, "get_job_event_store", return_value=mock_store),
        ...
        self.assertIs(run_io.await_args_list[0].args[0], callback_delivery.fetch_history)
        self.assertIs(run_io.await_args_list[1].args[0], callback_delivery.safe_request_json)
